 ...
 * 基于OSS的资源协议解析器
 * <p>
 * 基于OSS的资源协议解析器 {@link ProtocolResolver} 实现。 该解析器用于解析 {@code oss://} 协议。
 * {@link OssStorageResource} 用于与OSS服务进行交互。
 * </p>
 */
...
public class OssStorageProtocolResolver implements ProtocolResolver, ResourceLoaderAware {
    ...
    public Resource resolve(String location, ResourceLoader resourceLoader) {
        ...
        if (location.startsWith(PROTOCOL)) {
            // 创建OssStorageLocation对象
            OssStorageLocation ossStorageLocation = OssStorageLocation.ofLocation(location);
            // 返回OssStorageResource对象
            return new OssStorageResource(oss, ossTaskExecutor, ossStorageLocation);
        }
        return null;
    }
}
